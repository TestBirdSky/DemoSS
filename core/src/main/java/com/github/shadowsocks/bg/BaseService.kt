...
//                Firebase.analytics.logEvent("stop") { param(FirebaseAnalytics.Param.METHOD, tag) }
        ...
        fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
            Log.i(TAG,"onStartCommand ${data.state} --${Core.currentProfile}")
            val data = data
            if (data.state != State.Stopped) return Service.START_NOT_STICKY
            //拿到相关配置（该配置我们一般需要本地配置固定的数据、还有从云端获取数据（自由服务器、Firebase自己定义数据））
            val expanded = Core.currentProfile
            Log.i(TAG,"onStartCommand expanded=$expanded")
            this as Context
            if (expanded == null) {
            ...
            Log.i(TAG,"onStartCommand profile=$profile")
            try {
